...
MODULE global
    ...
    ! "params" determines the economy to be simulated. 
    	! It contains the values of: beta, rho_z, sigma_z_eps, sigma_lambda_eps . In order.
    ...
    REAL(DP), DIMENSION(MaxAge, na, nz, nlambda, ne) :: Cons,  Aprime
	...
	REAL(DP), DIMENSION(:,:,:,:,:), allocatable :: Cons_t,  Aprime_t
		...
		p(1) = Y_a_threshold_in
 		do zi=1,nz 
			...
			if (Y_a_threshold_in.lt.max_wealth) then
				...
				!print*, "Residual:", Y_a_res(a_aux(a_ind),p)
			end if 
 		end do 
 		...
 		agrid_t = agrid
!  		deallocate(agrid_t)
!  		allocate( agrid:t(1:na+1) )
!  		agrid_t = [agrid,2.0_dp]
!  		call Sort(na_t,agrid_t,agrid_t,agrid_t_ind)
	end if
...
!
	FUNCTION FOC_WH(aprimet)
		...
		FOC_WH   = (1.0_DP / (YGRID_t(ai,zi)  + wage*eff_un(age, lambdai,ei) - aprimet )  &
...
PROGRAM main
	...
	! Set parameters to be used in all simulations economy
		...
		!print*,  beta, rho_z, sigma_z_eps, sigma_lambda_eps
...
SUBROUTINE GOVNT_BUDGET
	...
	print*,'GBAR=',GBAR,'GBAR_K=', GBAR_K, "GBAR_W=", GBAR_W
	print*, 'Tau_K=', tauK, 'Tau_W=', tauW_at, "Threshold", Y_a_threshold
	print*, ' '
	...
	DO WHILE ( ( DBN_dist .ge. DBN_criteria ) .and. ( simutime .le. MaxSimuTime ) )
	    ...
	    IF (iter_indx .ge. update_period) THEN
	        ...
	        DO e1=1, ne
	             ...
	             NBAR= NBAR+ DBN1(age1, a1, z1, lambda1, e1) * eff_un(age1, lambda1, e1) 
	        ENDDO
	        ENDDO
	        ENDDO    
	        ENDDO    
...
SUBROUTINE COMPUTE_STATS
	...
	print*,'beta',beta,'rho_z',rho_z,'sigma_z',sigma_z_eps,'sigma_lam',sigma_lambda_eps
	print*,"Statistics"
...
SUBROUTINE WRITE_VARIABLES(bench_indx)
	...
	IF (bench_indx .gt. 0) then 
		...
		WRITE  (UNIT=2, FMT=*)  Wealth_Output, prct1_wealth , prct10_wealth
...
SUBROUTINE EGM_RETIREMENT_WORKING_PERIOD
	...
	REAL(DP), DIMENSION(na_t+1) :: EndoCons, EndoYgrid
    ...
    DO ai=1,na_t        
        ...
        Cons_t(age,ai,zi,lambdai,ei) = YGRID_t(ai,zi)+wage*eff_un(age,lambdai,ei)
	ENDDO ! ai
    ENDDO ! ei
    ENDDO ! zi
	ENDDO ! lambdai
    ...
    DO ai=1,na_t
		...
		if (abs(Wealth-Y_a_threshold).lt.1e-8) then 
			...
			EndoYgrid(ai) = agrid_t(ai) + EndoCons(ai) - wage*eff_un(age,lambdai,ei)
			...
			EndoYgrid(na_t+1) = agrid_t(ai) + EndoCons(na_t+1) - wage*eff_un(age,lambdai,ei)
	    	sw                = 1 
...
! 	end do
		...
		DO ai=tempai,na_t               
		    ...
		    Aprime_t(age,ai,zi,lambdai,ei) = YGRID_t(ai,zi) + wage*eff_un(age,lambdai,ei) - Cons_t(age, ai, zi, lambdai,ei)   
		                    
		    If (Aprime_t(age, ai, zi, lambdai,ei)  .lt. amin) then
            	Aprime_t(age, ai, zi, lambdai,ei) = amin         
		        
	            Cons_t(age,ai,zi,lambdai,ei) = YGRID_t(ai,zi) + wage*eff_un(age,lambdai,ei) - Aprime_t(age, ai, zi, lambdai,ei)      
        ...
        DO WHILE ( YGRID_t(ai,zi) .lt. EndoYgrid(1) )
			...
			brentvalue = brent( min(amin,YGRID_t(ai,zi))   ,  (amin+YGRID_t(ai,zi))/2.0_DP  ,  &
                             	& YGRID_t(ai,zi)  + wage*eff_un(age, lambdai,ei)*0.95_DP ,  &
