...
Contains 
  ...
  SUBROUTINE SIMULATION(ben_switch)
    ...
    INTEGER, PARAMETER  :: totpop=2400000, cohortsize=50000,  Max_Simu_Time=1100, LastTperiod=100
    ...
    do i=1,totpop
      ...
      panel_r(i)  = 1 + ( rr * (zgrid(panelz(i)))**mu * panela(i)**(mu-1)  - DepRate) ;
      panel_r_at(i) = (1 + ( rr * (zgrid(panelz(i)))**mu * panela(i)**(mu-1)  - DepRate)*(1.0_dp-tauK) )*(1.0_dp-tauW_at) ;
      
      if (panelage(i).lt.RetAge) then 
        panel_h(i)  = max(0.0_DP, 1.0_DP - (1.0_DP-gamma)*panel_c(i)/(gamma*psi*wage*eff_un(panelage(i),panellambda(i),panele(i))) )                                
        panel_ap(i) = Y_a(panela(i),zgrid(panelz(i))) + Y_h(panel_h(i),panelage(i),panellambda(i),panele(i),wage)  - panel_c(i)
      else 
        panel_h(i)  = 0.0_DP
        panel_ap(i) = Y_a(panela(i),zgrid(panelz(i))) + RetY_lambda_e(panellambda(i),panele(i))  - panel_c(i)
      end if 
      ...
      if (panel_ap(i).lt.agrid(1)) then
        ...
        if (panelage(i).lt.RetAge) then                   
          ...
          panel_c(i)  = Y_a(panela(i),zgrid(panelz(i))) + Y_h(panel_h(i),panelage(i),panellambda(i),panele(i),wage)  - panel_ap(i)
        else 
          panel_h(i) = 0.0_DP
          panel_c(i) = Y_a(panela(i),zgrid(panelz(i))) + RetY_lambda_e(panellambda(i),panele(i))  - panel_ap(i)
        end if 
      end if
